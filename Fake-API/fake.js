...
$.get(URL_ENDPOINT).then(data => { //jquery $ and a get url
    data.map(student => {// use map function to map out the people, kinda like cards
      ...
      <td><button onClick ="deleteUser(${student.id})">DeleteStudent</button></td>
      </tr>`)
    })
  })//call the names in the jquery
 ...
 *
 * ↓ YOUR CODE HERE ↓ */
$(`#submitStudent`).click(function (){//submit student function with the called name fullname and new assignment 
    $.post(URL_ENDPOINT,{
...
function deleteUser(id){
  ...
  }//delete method with the ajax url function
/*------------------------ HTTP Verb: UPDATE ------------------------*/
console.log(
  `-------------------------- 
 ...
 *
 * ↓ YOUR CODE HERE ↓ */
//var id = fullName, researchAssignment;
function updateUser() {
   let id = $(`#updateId`).val()//connected id by deleting all the stuff except the comment,and info came in

$.ajax(`${URL_ENDPOINT}/${id}`,{
  //$.ajax(`${URL_ENDPOINT}/${$(`#updateId`).val()}`)
 method: `PUT`,
  data: {
    fullName: $(`#updateName`),
    researchAssignment:$(`#researchAssignment`).val ,
  }
})//a function update method of put and you put the full name and research assignment to update
}
// Assuming you have the 'id' value defined somewhere
//var id = studentId;

// Create an object with the data to send in the request body
var newData = {
  name: "New Name",
  researchAssignment: "New Assignment"
};

// Set up the AJAX request
// $.ajax({
//   url: 'http://localhost:3000/studentRoster', // Replace with your API endpoint
//   method: 'PUT',
//   data: JSON.stringify(newData), // Convert data to JSON string
